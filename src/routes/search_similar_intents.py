...
def detect_intent(agent, session_id, text_input):
    try:
        ...
        sh.upload_audio_file(response=audi_response,
                             session_id=str(session_id))
        return {"success": True, "fulfillments": response_messages, "uuid": str(session_id)}, status.HTTP_200_OK
    except Exception:
...
def detect_intent_audio(agent, session_id, text_input):
    try:
        text_input = text_input[:255]  # dialog
        DIALOGFLOW_LANGUAGE_CODE = "en"
        ...
        output_audio_config = df.OutputAudioConfig(
            audio_encoding=df.OutputAudioEncoding.OUTPUT_AUDIO_ENCODING_MP3
        )
        detect_intent_request = df.DetectIntentRequest(
        ...
        #speech_text = response_messages[0]
        #audi_response = ts.convert_text_to_speech(speech_text)
        # sh.upload_audio_file(response=audi_response,session_id=str(session_id))
        audio_output = response.output_audio
        print(type(audio_output))
        return {"success": True, "fulfillments": response_messages, "uuid": str(session_id), "audio_content": str(audio_output)}, status.HTTP_200_OK
    except Exception:
        print("exception while input")
        print(text_input)
        print("Unexpected error:", sys.exc_info()[0])
...
@app.route("/detect-intent-audio", methods=["POST"])
@cross_origin()
@authorize()
def prepare_detect_intent_audio():
    """
    Returns the fulfillment text corresponding the intent
    """
    project_id = os.getenv(constant.PROJECT_ID, constant.DEFAULT_PROJECT_NAME)
    ...
    if session_id != "":
        ...
        SESSION_ID = uuid.uuid4()
        # Initializing Dialogflow pages.
        detect_intent_audio(agent, SESSION_ID, "Hi")
        detect_intent_audio(agent, SESSION_ID, product_name)

    resp = detect_intent_audio(agent, SESSION_ID, text_input)
    return resp
